...
class BeamSearchNode(object):
    ...
    def __init__(self, search, emb, lstm_out, final_hidden, final_cell, mask, sequence, logProb, length,all_logp=[]):
    ...
    def eval_p(self, alpha=0.0,lambda_reg=0.5):
        """ Returns score of sequence up to this node 
        ...
        """
        normalizer = (5 + self.length)**alpha / (5 + 1)**alpha
        squared_regularizer = sum([(-torch.log(prob))**2 for prob in self.all_logp])

        self.all_logp.append(self.logp)
        return (self.logp - (lambda_reg * squared_regularizer)) / normalizer