...
def get_args():
    ...
    parser.add_argument('--checkpoint-path', default='/Users/yuumu/Desktop/Master_Sem1/ATML/atmt_2023_a3/assignments/03/lexical/checkpoint_best.pt', help='path to the model file')
    ...
    parser.add_argument('--regulation', default=False, type=bool, help='whether apply regulation to the output')
    parser.add_argument('--diversity',default=False,type=bool,help='whether to apply diverse beam search')
    parser.add_argument('--n-best', default=1, type=int, help='number of hypotheses to return')
    parser.add_argument('--lamda', default=1, type=int, help='lamda for diverse beam search')
            ...
            for j in range(args.beam_size):
                ...
                if args.regulation:
                    ...
                    node = BeamSearchNode(searches[i], emb, lstm_out, final_hidden, final_cell,
                                        mask, torch.cat((go_slice[i], next_word)), log_p, 1,all_logp)
                    searches[i].add(-node.eval_p(args.alpha), node)
                else:
                    node = BeamSearchNode(searches[i], emb, lstm_out, final_hidden, final_cell,
        ...
        for _ in range(args.max_len-1):
                    ...
                    if next_word[-1] == tgt_dict.eos_idx:
                        if args.regulation:
                            node = BeamSearchNode(
                                ...
                                next_word)), node.logp, node.length,node.all_logp
                                )
                            search.add_final(-node.eval_p(args.alpha), node)
                        else:
                            node = BeamSearchNode(
                                search, node.emb, node.lstm_out, node.final_hidden,
                                node.final_cell, node.mask, torch.cat((prev_words[i][0].view([1]),
                    ...
                    # Add the node to current nodes for next iteration
                        ...
                        if args.regulation:
                            node = BeamSearchNode(
                                search, node.emb, node.lstm_out, node.final_hidden,
                                node.final_cell, node.mask, torch.cat((prev_words[i][0].view([1]),
                                next_word)), node.logp + log_p, node.length + 1,node.all_logp
                                )
                            search.add(-node.eval_p(args.alpha), node)
                        else:
                            node = BeamSearchNode(
                                search, node.emb, node.lstm_out, node.final_hidden,
                                node.final_cell, node.mask, torch.cat((prev_words[i][0].view([1]),
            ...
            for search in searches:
                if args.diversity:
                    ...
                    search.nodes.queue = [((n[0]-args.lamda*rank), n[1], n[2]) for rank, n in enumerate(sorted_search, start=1)]
                search.prune(args.diversity)
